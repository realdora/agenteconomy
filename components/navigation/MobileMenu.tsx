...
export function MobileMenu({ isOpen, onClose }: MobileMenuProps) {
  ...
  return (
    <nav className="tt-mobile-menu lg:hidden" aria-label="Mobile navigation">
      <div className="tt-mobile-menu-content">
        {navMenus.map((menu) => {
          if (menu.href) {
            return (
              <Link key={menu.label} href={menu.href} className="tt-mobile-nav-link" onClick={onClose}>
                {menu.label}
              </Link>
            );
          }
            ...
            <div key={menu.label} className="py-2">
              ...
              <div className="grid gap-2">
                {menu.panel?.items.map((item) => (
                  <Link key={item.href} href={item.href} className="tt-mobile-nav-card" onClick={onClose}>
