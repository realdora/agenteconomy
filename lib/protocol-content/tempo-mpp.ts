...
export const tempoMppContent: ProtocolDoc["content"] = [
  {
    ...
    body: [
      ...
      "The current total event count, unique payer and payee counts, and latest daily activity are published in the live table above, refreshed hourly from the Tempo RPC indexer's output. Those numbers are useful because they are concrete on-chain signals, but they should not be read as a complete market map. Tempo MPP coverage is narrower than the Dune-backed x402 coverage and should be interpreted as early Tempo-specific visible activity.",
  ...
  },
  {
    ...
    body: [
      "Agent Economy tracks Tempo MPP through the fields exposed in `data.json` and the Tempo-specific `tempo-data.json` summary. The route-level metric table uses `tempoMpp.totalEvents`, `tempoMpp.uniquePayers`, `tempoMpp.uniquePayees`, `tempoMpp.byType`, and `tempoMpp.daily` — the current values are in the live table above, each refresh carrying its own as-of stamp.",
