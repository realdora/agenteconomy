...
export const olasContent: ProtocolDoc["content"] = [
  {
    ...
    body: [
      ...
      "The current cumulative transaction count, chain count, and largest-chain split are published in the live table above, refreshed hourly from data.json with an explicit as-of stamp. The distribution is strongly Gnosis-dominant — a concentration that should be read as a property of the current source data, not as a claim that all Olas-related work happens on one chain. It says where this measured transaction family is visible today.",
    ],
    callout:
  ...
  },
  {
    ...
    body: [
      "The live table above carries the current cumulative Olas transaction count and the chain breakdown, with Gnosis as the largest chain by a wide margin. The remaining tracked chains — Optimism, Polygon, Base, Ethereum, Celo, Solana, and Arbitrum — carry much smaller counts. The point is not that every chain plays the same role; the point is that the source preserves the chain-level distribution instead of flattening it into a single headline number.",
      "Gnosis dominance is the most important interpretation note. With the large majority of the tracked Olas total settling on Gnosis (the exact current share is in the live table above), the Gnosis series drives the aggregate shape of the Olas chart. When the total moves, readers should expect Gnosis to explain most of that movement unless the chain mix changes materially. The dataset exposes that context directly through its largest-chain metric, and this guide keeps that same caveat visible.",
      "The dataset also carries a full weekly history. Weekly history is useful because autonomous-service activity can be lumpy. Services may run continuously, but their transaction counts can still rise and fall with market conditions, automation schedules, user participation, or changes in the underlying service set. A weekly series avoids pretending that one cumulative total explains the whole protocol. It gives analysts a way to see whether activity is persistent, seasonal, or driven by a small number of recent bursts.",
    ],
  },
  {
    eyebrow: "Methodology",
