-- Agent Economy owned production SQL; registry version 2026-09-16-legacy-scope-v1.
-- Preserves source 7895747 v2 by realdoraa.
WITH evm_transfers AS (
  ...
  WHERE tx_from IN (SELECT address FROM (WITH evm AS (
...
solana_transfers AS (
  ...
  WHERE tx_signer IN (SELECT address FROM (WITH sol AS (
  SELECT facilitator, address, chain
  FROM (VALUES
